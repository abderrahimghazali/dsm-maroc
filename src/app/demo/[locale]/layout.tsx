...
export async function generateMetadata({
  ...
  return {
    ...
    title: {
      default: `${c.meta.siteTitle} · ${c.meta.demoLabel}`,
      template: `%s · ${c.meta.siteTitle}`,
    },
    description: c.meta.description,
    openGraph: {
      locale: localeMeta[locale].code.replace("-", "_"),
      type: "website",
    },
...
export default async function DemoLayout({
  ...
  const nav = c.nav.map((item) => ({
    ...
    children: item.children?.map((ch) => ({
      ...ch,
      href: demoHref(locale, ch.href),
    })),
    featured: item.featured
      ? { ...item.featured, href: demoHref(locale, item.featured.href) }
      : undefined,
  }));

  return (
    // Root layout of the demo portal: <html> carries the demo locale's lang and dir (server-rendered).
    <html
      lang={meta.code}
      dir={meta.dir}
    ...
    >
      <head>
        <Script id="dsm-theme-init" strategy="beforeInteractive">
          {themeInitScript}
        </Script>
      </head>
      <body id="top" className="flex min-h-dvh flex-col bg-canvas text-ink">
        <ThemeProvider>
          <LocaleProvider locale={locale}>
            <DemoNotice
              text={c.meta.demoNotice}
              linkLabel={c.meta.demoNoticeLink}
            />
            <OfficialBanner />
            <Header
              entity={entity}
            ...
            <Footer
              ...
              ecosystem={c.footer.ecosystem.map((l) => ({
                ...l,
                external: true,
              }))}
              columns={c.footer.columns.map((col) => ({
                ...
                links: col.links.map((l) => ({
                  label: l.label,
                  href: demoHref(locale, l.href),
                })),
              }))}
              bottomLinks={c.footer.bottomLinks.map((l) => ({
                label: l.label,
                href: demoHref(locale, l.href),
              }))}
              social={[
